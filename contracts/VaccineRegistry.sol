...
contract VaccineRegistry {
    ...
    mapping(string => int256) vaccineTemp; // thresold temperature
    ...
    mapping(uint256 => mapping(string => address)) administratedVaccines;
    ...
    function getRequiredCount() public view returns (int256) {
        return
            int256(ben.getRegisteredCount()) -
            int256(vaccinatedCount + availableVaccines);
    }

    function registerVaccineLot(
        string memory _vaccineLotId,
        int256 _quantity,
        int256 thresoldTemp
    ) public {
    ...
    function monitor(string memory _vaccineLotId, int16 temperature) public {
        ...
        monitoredData[_vaccineLotId].push(
            MonitoredData(temperature, block.timestamp)
        );
    }

    function getMonitoredData(string memory _vaccineLotId)
        public
        view
        returns (MonitoredData[] memory)
    {
        return monitoredData[_vaccineLotId];
    }

    function checkLegitmacy(string memory _vaccineLotId)
        public
        view
        returns (bool)
    {
